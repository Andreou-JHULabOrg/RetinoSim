...
int ReadVideo(vector<Mat> &frames) {
    VideoCapture video(
        "/Users/susanliu/Documents/AndreouResearch/videos/cat_jump.mp4");
    if (!video.isOpened()) {
...
Mat AddNormrnd(Mat frame, double mean, Mat sigma) {
    sigma.convertTo(sigma, CV_32F);
    Mat result = frame.clone();
    for (int k = 0; k < rows; k++) {
        for (int i = 0; i < cols; i++) {
            double sig = sigma.at<float>(k, i);
            result.at<float>(k, i) = frame.at<float>(k, i) + Normrnd(mean, sig);
        }
    }
    return result;
}
...
void processFrames(Parameter params) {
    ...
    Mat pixel_fe_noise(rows, cols, CV_32F, 0.0);
    ...
    double noise =
        ...
        average_current;
    Mat pix_shot_rate, pixel_fe_noise_past;
    for (int k = 1; k < numFrames; k++)  // starting from the second frame
    {
        ...
        if (params.enable_pixel_variance) {
            if (k == 1) {  // needs to calculate past noise first
                ...
                pixel_fe_noise_past =
                    AddNormrnd(pixel_fe_noise, 0.0, pix_shot_rate);
            }
            pix_shot_rate = noise * (max - curFrame);
            pixel_fe_noise = AddNormrnd(pixel_fe_noise, 0.0, pix_shot_rate);
            add(frames[k - 1], pixel_fe_noise_past, pastFrame, Mat(),
                CV_32F);
        ...
        curFrame.convertTo(frames[k], CV_8U);
        I_mem_p = I_mem.clone();
