function [TD, eventFrames, rng_settings, dbgFrames, OMSNeuron, eventCount] = StNvsModel_shotNoise(input_video, params)
...
leak_rate = normrnd(params.leak_ba_rate, sqrt(params.leak_ba_rate/2), size(grayFrames(:,:,1)));
...
%           end
    ...
    %Rectify signals
    onIdx = (frame.opl_str) > (threshold_array.bc+ params.bc_offset);
    offIdx = (frame.opl_str) < (threshold_array.bc- params.bc_offset);
...
%     high_pass_response.off = ((params.hpf_gc_tc)*offNeuron.state_(offIdx) + (params.hpf_gc_tc)*(diffOPL(offIdx)));
    ...
    onNeuron.state      = max(offNeuron.state, 0);
